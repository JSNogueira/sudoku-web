...
public class SudokuService {
    ...
    public boolean verificarErros(List<Celula> celulas, int[][] tab){
        int[][] tabCompleto = iniciarTabuleiroCompleto(celulas);
        for(int i=0; i<9; i++){
            for(int j=0; j<9; j++){
                if(tab[i][j] != 0){
                    if (tab[i][j] != tabCompleto[i][j]) {
                        return false;
                    }
                }
            }   
    ...
    public int[][] iniciarTabuleiroCompleto(List<Celula> celulas){
        int[][] tabuleiro = new int[9][9];
    ...
    public boolean verificarTabuleiroCompleto(int[][] tab){
        for(int i=0; i<9; i++){
            for(int j=0; j<9; j++){
                if(tab[i][j] == 0){
                    return false;
